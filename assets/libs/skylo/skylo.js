...
;(function ( $, window, document, undefined ) {
    ...
    Plugin.prototype.setState = function(state) {
        var currentState = ((this.options.state) ? ("progress-bar-"+this.options.state+" bar-"+this.options.state):"");
        if(currentState != "") {
            $('.progress.skylo .bar').removeClass(currentState);
        }

        if(state !== 'info' && state !== 'success' && state !== 'danger' && state !== 'warning') {
            //set to default i.e; ignore op
        }
        $('.progress.skylo .bar').addClass("progress-bar-"+state+" bar-"+state);
